...
//   5. FUNDACIONAL — base liviana sobre la que agent-media.js extiende
//
// vs agent-media.js:
...
//   Poh et al. (2010) — HR via ICA
//   De Haan & Jeanne (2013) — CHROM para rPPG robusto
//   Mcduff et al. (2014) — Stress via HR variability webcam
//   Kaliouby & Robinson (2005) — Engagement via face presence
//   Cummins et al. (2015) — Speech biomarkers for depression
//
// METRICAS (nombres canonicos del METRIC_DICTIONARY):
//   og_cam_present            bool  — camara activa
//   og_cam_presence_pct       ratio — pct tiempo con contenido visual
//   og_cam_blackout_count     count — episodios sin deteccion
//   og_cam_blackout_max_ms    ms    — blackout mas largo
...
//   og_mic_present            bool  — microfono activo
//   og_mic_db_mean            dB    — volumen ambiental medio
//   og_mic_db_cv              ratio — variabilidad volumen
//   og_mic_silence_pct        ratio — pct en silencio (<30dB)
//   og_mic_speech_episodes    count — episodios de habla (>50dB)
//   og_mic_peak_db            dB    — pico maximo de audio
// ================================================================
...
var state = {
    consent_cam: false, consent_mic: false, hw_tier: 'none',
    _stream_cam: null, _stream_mic: null, _video: null,
    _canvas: null, _ctx: null, _audio_ctx: null, _analyser: null,
    _cam_iv: null, _mic_iv: null,
    frames_total: 0, frames_present: 0,
    blackout_episodes: [], _blackout_start: null,
    luminance_samples: [], green_samples: [],
    db_samples: [], silence_samples: 0,
    speech_episodes: 0, _in_speech: false, peak_db: 0,
    _t0: 0, active: false
};
...
async function startCam() {
    try {
        ...
        console.warn('[og-media] cam:', e.message);
...
function sampleFrame() {
    ...
    try {
        ...
        var len = data.length;
        var sumG = 0, sumLum = 0, nonBlack = 0;
    ...
    try {
        ...
        state._mic_iv = setInterval(function() {
            ...
            if (typeof ZYKOS !== 'undefined' && ZYKOS._pushRaw && state.db_samples.length % 5 === 0) {
                ZYKOS._pushRaw('og_audio', { t_ms: Math.round(performance.now()-state._t0), db: +(db.toFixed(1)) });
            }
        }, SAMPLE_MS);
    } catch(e) {
        console.warn('[og-media] mic:', e.message);
...
var agent = {
    ...
    start: async function() {
        ...
        state._t0=performance.now(); state.active=true;
    ...
    collect: function() {
        ...
        var blackoutMax = state.blackout_episodes.length > 0
            ? Math.max.apply(null, state.blackout_episodes.map(function(e){return e.duration_ms;})) : 0;
        var dbN = state.db_samples.length;
        return {
            ...
            og_mic_silence_pct:        state.consent_mic && dbN>0 ? +(state.silence_samples/dbN).toFixed(3) : null,
    ...
    pause:  function() { /* blackout es dato clinico — no pausar */ },
    resume: function() {},
